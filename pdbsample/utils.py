...
def run(executable, args=None, stdin=None, stdout=None, stderr=None):
    if args is None:
        args = []
    if stdin is None:
        stdin = []
    ...
    p = subprocess.Popen(
        command, stdin=pstdin, stdout=pstdout, stderr=pstderr, encoding="utf8"
    )
...
def parallel(title, func, dictionary, processes=None):
    ...
    for key, value in dictionary.items():
        pool.apply_async(
            func, args=(key, value), callback=callback, error_callback=error_callback
        )
    pool.close()
    pool.join()
    progress_bar.finish()
